...
serves a built-in web console plus the JSON/streaming endpoints below.

## Endpoints

### `GET /`

The self-contained chat console (`static/index.html`) — a role/identity
selector, streaming chat, and a live audit-trail panel. No build step, no Node.
Open <http://localhost:8000> after starting the server.

### `GET /health`

`{ status, mcp_connected, tools }` — confirms the MCP session is live and lists
the tool surface. Used by the console's connection dot.
