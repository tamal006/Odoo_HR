...
# ---------------------------------------------------------------------------
# 1b. Hard RBAC gate — enforced in code, not just in the system prompt.
# ---------------------------------------------------------------------------
# The system prompt tells the model what it may not do; this is the net that
# holds even if the model is jailbroken past it. Mirrors the README role table
# and Global Restrictions — it invents no new policy, it just makes the
# existing policy actually enforced at the tool-execution boundary.


def rbac_denial(role: str, tool_name: str, tool_args: dict[str, Any]) -> str | None:
    """Return a denial reason if *role* must not run this tool, else ``None``.

    Applied before any tool executes (and before the approval copilot), so a
    denied call never reaches the MCP server.
    """
    # Deletions are forbidden for everyone (Global Restriction).
    if tool_name == "delete_record":
        return "delete_record is disabled for all roles."
    ...
    if is_approval_action(tool_name, tool_args):
        return "Employees cannot approve or reject leave requests."
    if tool_name in ("create_record", "update_record") and tool_args.get("model") == "hr.employee":
        return "Employees cannot create or modify employee records."
    # Other writes (e.g. submitting one's own leave) are allowed here; row-level
    # ownership is enforced by Odoo's own ACLs on the real server.
    return None
...
async def gather_approval_context(
    ...
    # -- 5. AI risk assessment (the headline) ------------------------------
    #    A single scored, explainable recommendation folding balance + team
    #    overlap + burnout signal together. Best-effort: if the connected
    #    server doesn't expose it, we degrade to the raw context above.
    risk_text = await _call_tool_safe(
        session,
        "approval_risk",
...
def _format_risk(risk_json: str) -> str:
    """Render the approval_risk JSON into a headline block, or pass raw text
    through if it isn't the expected JSON (e.g. real server lacks the tool)."""
    try:
        r = json.loads(risk_json)
    except (json.JSONDecodeError, TypeError):
        return risk_json or "(risk model unavailable)"
    if "recommendation" not in r:
        return risk_json
    lines = [
        f"  Recommendation: {r['recommendation']}  "
        f"(risk {r.get('risk_score')}/100, confidence {r.get('confidence')})",
    ]
...
def format_copilot_context(context: dict[str, str], action: str) -> str:
    ...
    sections = [
        ...
        "🎯 AI Risk Assessment",
        "─" * 40,
        _format_risk(context.get("risk_assessment", "")),
        "",
