    ...
    try {
      ...
      if (data.length < 3) {
        ...
        const MOCK_LEAVES = [
          ...
          { id: 304, employeeId: 105, employeeName: "Maya Lin", leaveType: "Annual Leave", leaveTypeId: 1, dateFrom: new Date(now + 86400000 * 5).toISOString().slice(0, 10), dateTo: new Date(now + 86400000 * 7).toISOString().slice(0, 10), numberOfDays: 2, state: "confirm", stateLabel: "Pending", reason: "Conference attendance" },
        ];
        data = [...data, ...MOCK_LEAVES];
      }
